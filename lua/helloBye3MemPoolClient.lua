...
void HelloBye3MemPool_Client_config(uint32_t srcIP, uint16_t dstPort);

void *HelloBye3MemPool_Client_connect(void *obj, struct rte_mbuf **inPkts, unsigned int inCount,
...
void *HelloBye3MemPool_Client_process(void *obj, struct rte_mbuf **inPkts, unsigned int inCount,
...
function mod.init()
	ret = {}
	ret.obj = ffi.C.HelloBye3MemPool_Client_init()
...
--		log:info("helloBye.process() called (>0 packets)")
		...
		local ba = ffi.C.HelloBye3MemPool_Client_process(obj.obj, inPkts, inCount, obj.sbc,
		obj.fbc)
		...
		ffi.C.HelloBye3MemPool_Client_getPkts(ba, obj.sbufs.array, obj.fbufs.array)
...
function mod.connect(mempool, obj, srcIP, dstIP, srcPort, dstPort, ident, bSize)
	...
	for i = 1,bSize do
		...
		local bAC = ffi.C.HelloBye3MemPool_Client_connect(obj.obj, bufArray.array + (i-1), 1, obj.sbc,
		obj.fbc, srcIP, dstIP, srcPort, dstPort, ident +( i-1))

		local freeBufs = memory.bufArray(obj.fbc[0])

		ffi.C.HelloBye3MemPool_Client_getPkts(bAC, sendBufsAll.array + (i-1), freeBufs.array)
...
function mod.free(obj)
	ffi.C.HelloBye3MemPool_Client_free(obj.obj)
end
